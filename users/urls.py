...
urlpatterns = [
    ...
    path('password-change/done/', PasswordChangeDoneView.as_view(
        template_name='users/order/password_change_done.html', 
        extra_context={'title': 'Пароль успешно изменен!'}), name='password_change_done'),

    path('password-reset/', PasswordResetView.as_view(
        template_name='users/order/password_reset_form.html', 
        email_template_name='users/order/password_reset_email.html',
    ...
    path('password-reset/done/', PasswordResetDoneView.as_view(
        template_name='users/order/password_reset_done.html',
        extra_context={'title': 'Сброс пароля'}), 
        name='password_reset_done'),
    path('password-reset/<uidb64>/<token>/', PasswordResetConfirmView.as_view(
        template_name="users/order/password_reset_confirm.html",
        success_url=reverse_lazy('users:password_reset_complete')), name='password_reset_confirm'),
    path('password-reset/complete/', PasswordResetCompleteView.as_view(
        template_name="users/order/password_reset_complete.html"), name='password_reset_complete'),
